...
export default function Home() {
  ...
  const [showInvitePopup, setShowInvitePopup] = useState(false); // invite info popup
  ...
  useEffect(() => {
    try {
      ...
      // Handle lang param from invite/share links - set UI language
      const langParam = urlParams.get('lang');
      if (langParam && LANGS.find(l => l.code === langParam)) {
        setInviteMsgLang(langParam);
        // Always set myLang from invite (this is the chat language for this session)
        setMyLang(langParam);
        // Set UI language to invite language if no saved prefs
        if (!saved) {
    ...
    <div style={S.page}>
      <div style={S.center}>
        ...
        <div style={S.card}>
          ...
          {/* Info link for invite recipients */}
