    ...
    <div role="dialog" aria-modal="true"
      ...
      <div style={{ width: '100%', maxWidth: 560, background: card, borderRadius: '20px 20px 0 0', border: bordo, borderBottom: 'none', fontFamily: FONT, padding: 16, paddingBottom: 'calc(16px + env(safe-area-inset-bottom))' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          <span style={{ flex: 1, fontSize: 12, fontWeight: 500, color: 'rgba(255,255,255,0.55)', letterSpacing: 0.5 }}>{tt('sharedWithBartalk', 'Hai condiviso')}{fonte ? ` · ${fonte}` : ''}</span>
        ...
        <div style={{ fontSize: 15, fontWeight: 500, color: '#fff', lineHeight: 1.35 }}>{titolo}</div>
        {sintesi && <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.6)', marginTop: 5, lineHeight: 1.45 }}>{sintesi}</div>}

        <div style={{ fontSize: 12, fontWeight: 500, color: 'rgba(255,255,255,0.5)', margin: '14px 0 8px' }}>{tt('sharedWhatToDo', 'Cosa vuoi farne?')}</div>
        <button onClick={() => onParlane({ titolo, sintesi: sintesi || (fonte ? `da ${fonte}` : ''), url: condiviso.url })}
          style={{ width: '100%', padding: 13, borderRadius: 13, border: 'none', cursor: 'pointer', background: accent, color: '#04121c', fontWeight: 500, fontSize: 15, fontFamily: FONT, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
          ...
          <button onClick={() => onLife?.('impara', titolo)}
            style={{ flex: 1, padding: 11, borderRadius: 12, border: bordo, background: 'transparent', color: '#fff', fontWeight: 500, fontSize: 13, cursor: 'pointer', fontFamily: FONT }}>
