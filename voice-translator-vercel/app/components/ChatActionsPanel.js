...
function ChatActionsPanel({
  ...
  // Export / share result
  // b.617 — il titolo del risultato era il nome INGLESE della tavola
  // («Summary» dentro un'app in italiano). `t` di riserva torna la chiave:
  // in quel caso si ripiega sul nome, non si mostra la chiave.
  const nomeAzione = useCallback((id) => {
    const voce = CHAT_ACTIONS.find(a => a.id === id);
    if (!voce) return '';
    const tradotto = t(voce.nameKey);
    return (tradotto && tradotto !== voce.nameKey) ? tradotto : voce.name;
  }, [t]);

  const shareResult = useCallback(async () => {
    if (!result?.text) return;
    const title = nomeAzione(result.action) || 'Report';
  ...
  const actionName = nomeAzione(result?.action);   // b.617 — tradotto
