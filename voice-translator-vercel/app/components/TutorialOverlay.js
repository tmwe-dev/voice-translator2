  ...
  return (
    <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, zIndex:9999,
      ...
      <div style={{maxWidth:360, width:'100%', textAlign:'center'}} onClick={e => e.stopPropagation()}>
        ...
        <div style={{display:'flex', gap:8, justifyContent:'center', marginBottom:24}}>
          {[0,1,2,3].map(i => (
            <div key={i} style={{width:tutorialStep===i?24:8, height:8, borderRadius:4,
              background:tutorialStep===i?S.colors.accent3:S.colors.textMuted,
              transition:'all 0.3s'}} />
          ))}
        </div>
        {/* Step content */}
        ...
        <div style={{fontSize:20, fontWeight:700, marginBottom:8, color:S.colors.textPrimary}}>
          {L(`tutorialStep${tutorialStep+1}Title`)}
        </div>
        <div style={{fontSize:14, color:S.colors.textTertiary, lineHeight:1.6, marginBottom:32}}>
          {L(`tutorialStep${tutorialStep+1}Desc`)}
        </div>
        {/* Navigation */}
        <div style={{display:'flex', gap:12, justifyContent:'center'}}>
          <button style={{padding:'10px 24px', borderRadius:14, background:'none',
            border:`1px solid ${S.colors.overlayBorder}`, color:S.colors.textMuted,
            fontSize:13, cursor:'pointer', fontFamily:FONT}}
            onClick={() => setShowTutorial(false)}>
            {L('skip')}
          </button>
          <button style={{padding:'10px 32px', borderRadius:14, border:'none',
            background:`linear-gradient(135deg, ${S.colors.accent3}, ${S.colors.accent3})`, color:S.colors.textPrimary,
            fontSize:14, fontWeight:600, cursor:'pointer', fontFamily:FONT,
            boxShadow:`0 4px 16px ${S.colors.accent3Bg}`}}
            onClick={() => {
